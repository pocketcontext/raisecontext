...
HOST = "raise.pocketcontext.com"
IMAGE = "ghcr.io/pocketcontext/raisecontext:latest"
...
def deploy():
    ...
    if len(matches) != 1:
        raise RuntimeError("Expected exactly one RaiseContext container; refusing an ambiguous update")
    container = matches[0]
    if container["Config"]["Image"] != IMAGE and not re.fullmatch(
            r"ghcr\.io/pocketcontext/raisecontext@sha256:[0-9a-f]{64}", container["Config"]["Image"]):
        raise RuntimeError("RaiseContext image does not match the fixed deployment target")
    ...
    try:
        ...
        if state["Running"] or state["ExitCode"] != 0 or state.get("OOMKilled"):
            raise RuntimeError("RaiseContext did not stop gracefully; refusing to replace it")
        ...
        try:
            ...
            print("RaiseContext recovery failed; operator intervention required", file=sys.stderr)
        raise
...
def main():
    ...
    with open("/run/lock/deploy-raisecontext.lock", "a") as lock:
