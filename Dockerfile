# RaiseContext image for Basecamp ONCE: HTTP on port 80, GET /up, all state under /storage.
...
LABEL org.opencontainers.image.title="RaiseContext" \
      org.opencontainers.image.description="Startup fundraising operated through a coding agent: PocketContext server, fundraising migrations and hooks, Litestream replication" \
      org.opencontainers.image.source="https://github.com/pocketcontext/raisecontext" \
