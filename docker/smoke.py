#!/usr/bin/env python3
"""Checks of the RaiseContext container image. Python 3 standard library and the docker CLI only.
...
def superuser_token(base, email, password):
    ...
    check(True, 'superuser login with RAISECONTEXT_SUPERUSER_EMAIL and RAISECONTEXT_SUPERUSER_PASSWORD')
...
def write_record(client, user_id):
    record = json.loads(client.run('create', 'organizations', json.dumps({'name': 'Synthetic image investor'})))
    return record['id'], None


def check_records(client, organization, unused):
    rows = client.sql(f"SELECT name FROM organizations WHERE id = '{organization}'")
    check(rows == [['Synthetic image investor']], 'synthetic investor survives persistence and restore')
...
def once_env(extra=None):
    ...
    env = {
        'BASE_URL': 'https://raise.example.test', 'SECRET_KEY_BASE': secret(secrets.token_hex(32)), 'DISABLE_SSL': 'true', 'NUM_CPUS': '2',
...
def smoke(image, tmp, run_id):
    name, volume = f'rc-smoke-{run_id}', f'rc-smoke-{run_id}'
    ...
    token = superuser_token(base, env['RAISECONTEXT_SUPERUSER_EMAIL'], env['RAISECONTEXT_SUPERUSER_PASSWORD'])
    ...
    check(settings['rateLimits']['enabled'] is True, "rate limits are enabled by the image's default RAISECONTEXT_RATE_LIMITS=true")
...
def config(image, tmp, run_id):
    ...
    expect_startup_error(image, 'a superuser email without a password', {'LITESTREAM_DISABLED': 'true', 'RAISECONTEXT_SUPERUSER_EMAIL': 'operator@example.test'},
                         ['RAISECONTEXT_SUPERUSER_PASSWORD'])

    expect_startup_error(image, 'a Google client ID without a secret',
                         {'LITESTREAM_DISABLED': 'true', 'RAISECONTEXT_GOOGLE_CLIENT_ID': 'image-test.apps.googleusercontent.com'},
                         ['RAISECONTEXT_GOOGLE_CLIENT_SECRET'])
    expect_startup_error(image, 'a Google secret without a client ID',
                         {'LITESTREAM_DISABLED': 'true', 'RAISECONTEXT_GOOGLE_CLIENT_SECRET': secret(secrets.token_urlsafe(24))},
                         ['RAISECONTEXT_GOOGLE_CLIENT_ID'])
    ...
    run_app(image, name, f'rc-config-{run_id}', env)
    time.sleep(20)
    running, code = state(name)
    text = check_logs(name)
...
def restore(image, tmp, run_id):
    network, minio, bucket = f'rc-drill-{run_id}', f'rc-drill-minio-{run_id}', 'raisecontext-drill'
    ...
    superuser_token(base, env['RAISECONTEXT_SUPERUSER_EMAIL'], env['RAISECONTEXT_SUPERUSER_PASSWORD'])

    step('a write immediately before docker stop must reach the replica through the shutdown sync')
    late = json.loads(client.run('create', 'organizations', json.dumps({'name': 'Written before stop'})))['id']
    stop(second)
    ...
    check_records(client, organization, unused)
    rows = client.sql(f"SELECT name FROM organizations WHERE id = '{late}'")
    check(rows == [['Written before stop']], f'the organization written before the stop was restored (rows: {rows})')
    check_logs(third)
    stop(third)
    check_logs(minio)
...
def main():
    ...
    failed = True
    with tempfile.TemporaryDirectory(prefix='raisecontext-image-') as tmp:
        try:
