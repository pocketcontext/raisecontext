...
def main():
    ...
    env = smoke.once_env({'RAISECONTEXT_RATE_LIMITS': 'true',
                          'RAISECONTEXT_TRUSTED_PROXY_HEADER': 'X-Forwarded-For'})
    ...
    for values in [
        {'RAISECONTEXT_GOOGLE_CLIENT_ID': 'synthetic-client'},
        {'RAISECONTEXT_GOOGLE_CLIENT_SECRET': 'synthetic-secret'},
        {'RAISECONTEXT_GOOGLE_CLIENT_ID': 'has whitespace', 'RAISECONTEXT_GOOGLE_CLIENT_SECRET': 'synthetic-secret'},
    ]:
        with tempfile.TemporaryDirectory(prefix='raisecontext-invalid-config-') as tmp:
            clean_env = {k: v for k, v in os.environ.items() if not k.startswith('RAISECONTEXT_')}
