...
function request(e) {
  ...
  if (!e.auth && e.request.header.get("Authorization")) {
    throw new UnauthorizedError("The RaiseContext session is invalid or expired.");
  }
  if (isUser(e.auth)) {
    ...
    if (current.getBool("disabled") || current.tokenKey() !== e.auth.tokenKey()) {
      throw new ForbiddenError("This RaiseContext account is disabled or its session was revoked.");
    }
  }
  return e.next();
}
...
function remove() {
  throw new ForbiddenError("RaiseContext accounts cannot be deleted; disable the account to preserve assignments and history.");
}
