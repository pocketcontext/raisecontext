...
function settings(app) {
  ...
  const header = env("RAISECONTEXT_TRUSTED_PROXY_HEADER");
  if (header) {
  ...
  const limits = env("RAISECONTEXT_RATE_LIMITS");
  if (limits === "true") {
    ...
    console.log("deploy: RAISECONTEXT_RATE_LIMITS must be true or false; rate limit settings left alone");
  }
}

function googleOAuth(app) {
  const clientId = String($os.getenv("RAISECONTEXT_GOOGLE_CLIENT_ID") || "");
  const clientSecret = String($os.getenv("RAISECONTEXT_GOOGLE_CLIENT_SECRET") || "");
  if (!clientId && !clientSecret) return;
  if (!clientId.trim() || !clientSecret.trim()) {
    throw new Error("RAISECONTEXT_GOOGLE_CLIENT_ID and RAISECONTEXT_GOOGLE_CLIENT_SECRET must be set together");
  }
  if (/\s/.test(clientId) || /\s/.test(clientSecret)) {
    throw new Error("RaiseContext Google OAuth credentials must not contain whitespace");
  }
  try {
    // PocketBase's system migrations already created users before bootstrap
    // completes. RaiseContext migrations deliberately preserve its OAuth options.
    ...
    throw new Error("Could not apply RaiseContext Google OAuth configuration; server startup stopped");
  }
}
