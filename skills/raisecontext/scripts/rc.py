#!/usr/bin/env python3
"""Command-line client for a RaiseContext fundraising server. Python 3 standard library only.

Configuration comes from three environment variables:
  RAISECONTEXT_URL             server address, for example https://raise.example.com
  RAISECONTEXT_USER_EMAIL     email of an account in the `users` collection
  RAISECONTEXT_USER_PASSWORD  password of that account (optional with Google login)
...
ENV = ['RAISECONTEXT_URL', 'RAISECONTEXT_USER_EMAIL', 'RAISECONTEXT_USER_PASSWORD']
...
def config(names=ENV[:2]):
    ...
    if parsed.scheme not in ('http', 'https') or not parsed.hostname or parsed.username or parsed.password or parsed.query or parsed.fragment:
        raise Fail(2, 'RAISECONTEXT_URL must be an HTTP(S) URL without credentials, query, or fragment')
    if parsed.scheme == 'http' and parsed.hostname not in ('localhost', '127.0.0.1', '::1'):
        raise Fail(2, 'Use HTTPS for a remote RaiseContext server')
...
def cache_file(cfg):
    ...
    return Path(base) / 'raisecontext' / (key + '.json')
...
def send(cfg, method, path, body=None, token=None, timeout=TIMEOUT):
    ...
    try:
        ...
        if 300 <= status < 400:
            raise Fail(1, f'HTTP {status}: the server redirects to {error.headers.get("Location")}. Set RAISECONTEXT_URL to the final address.')
...
def login(cfg):
    if not cfg.get('password'):
        raise Fail(2, 'Set RAISECONTEXT_USER_PASSWORD for password login, or run rc.py login --google for browser sign-in.')
    ...
    if status != 200 or not isinstance(data, dict) or 'token' not in data:
        raise Fail(1, f'login as {cfg["email"]} failed: HTTP {status}\n{dump(data)}\nCheck the three RAISECONTEXT_ variables with the user. User credentials only.')
...
def auth_session(cfg, data, method):
    ...
    if (not isinstance(token, str) or not token or not isinstance(record, dict) or record.get('collectionName') != 'users'
        ...
        raise Fail(1, 'Authentication returned an unexpected identity; no session saved. Check RAISECONTEXT_USER_EMAIL.')
...
def google_login(cfg, port=8765, timeout=180):
    ...
    if status != 200 or not oauth.get('enabled') or not provider:
        raise Fail(1, 'Google OAuth is not enabled on this RaiseContext server.')
    ...
    class Callback(http.server.BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if not valid:
                ...
                outcome['error'] = 'Google sign-in was denied or cancelled; run rc.py login --google to retry.'
    ...
    if not outcome:
        raise Fail(1, 'Google sign-in timed out; run rc.py login --google to retry.')
    if 'error' in outcome:
...
def call(cfg, method, path, body=None):
    ...
    if session.get('method') == 'google' and (oauth_refresh_needed(session) or path == '/api/collections/users/auth-refresh'):
        ...
        if status != 200:
            raise Fail(1, f'Google session could not be refreshed (HTTP {status}); run rc.py login --google again.')
    ...
    if cached and 400 <= status < 500 and status != 409 and (status == 401 or token_rejected(cfg, session['token'])):
        if session.get('method') == 'google':
            raise Fail(1, 'Google session was rejected; run rc.py login --google again.')
        session = login(cfg)
        status, data = send(cfg, method, path, body, session['token'])
    return status, data
...
def check(cfg):
    ...
    for line in differences:
        say(line, sys.stdout)
    say('The server is authoritative: run `rc.py schema` and follow the server\'s error messages where the reference files disagree. '
...
def run(args):
    ...
    if args.command in ('create', 'update'):
        ...
        for entry in requests:
            ...
            if method not in ('POST', 'PATCH') or not isinstance(url, str) or not re.fullmatch(r'/api/collections/(organizations|people|rounds|opportunities|opportunity_participants|introductions|activities|notes|messages|drafts|commitments|receipts)/records(?:/[a-z0-9]{15})?', url):
...
def parse(argv):
    ...
    parser = argparse.ArgumentParser(prog='rc.py', parents=[pretty], description='RaiseContext fundraising client. Reads with SQL, writes through the records API. There is no delete command: users cannot delete records.',
...
def main():
    try:
        ...
        say(f'rc.py: unexpected {type(error).__name__}: {error}')
        return 1
